...
const reconnectCoordinator = createRemoteReconnectCoordinator({
  ...
  host: {
    ...
    acquireLease: (host) => remoteHostRuntime.acquirePreparedHost(host),
  },
  terminal: {
    probe: probeRemoteTmuxSession,
    reattach: reattachRemotePty,
    hasPty,
  },
  feedback: {
    toast: emitToast,
...
// The reconnect/probe orchestration now lives in remote-reconnect.ts. These
// exported delegators preserve the signatures external callers (index.ts) and
// the session-manager test suite bind to; each forwards to the coordinator.
export async function reconnectRemoteSession(id: string): Promise<void> {
  return reconnectCoordinator.reconnectRemoteSession(id)
}

export async function attemptAutoReconnect(id: string): Promise<AttemptOutcome> {
  return reconnectCoordinator.attemptAutoReconnect(id)
}
...
export async function probePendingSessionsOnHost(
  ...
  return reconnectCoordinator.probePendingSessionsOnHost(hostId, stateHint)
}
