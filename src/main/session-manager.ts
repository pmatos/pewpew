...
// Bind a `GitRunner` to `projectPath` and delegate to git-refs so the local
// branch-existence check lives in exactly one place.
async function branchExists(projectPath: string, branchName: string): Promise<boolean> {
  return localBranchExists(
    (argv) =>
      execFileAsync('git', ['-C', projectPath, ...argv], { timeout: 5000 }).then(({ stdout }) => ({
